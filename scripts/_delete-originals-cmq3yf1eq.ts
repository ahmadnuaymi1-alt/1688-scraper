/**
 * Headless delete-originals for cmq3yf1eq000jw2kcemhuhsp9.
 * Replicates /api/products/[id]/delete-originals/route.ts behavior.
 */
...
(async () => {
  ...
  try {
    ...
    console.log(`Found ${originals.length} original .alicdn.com images to delete (of ${candidates.length} candidates)`);
    if (originals.length === 0) {
      const keptCount = await prisma.productImage.count({ where: { productId: PID, imageType: null, keep: true } });
      console.log(`deleted=0  kept=${keptCount}`);
      return;
    }
    ...
    if (paths.length > 0) {
      try {
        ...
        console.warn(`supabase remove threw: ${err instanceof Error ? err.message : String(err)}`);
      }
    }

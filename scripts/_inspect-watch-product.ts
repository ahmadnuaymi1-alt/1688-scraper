...
(async () => {
  ...
  const prod = await p.product.findUnique({
    where: { id: PID },
    select: { title: true, productType: true, optionNames: true, descriptionHtml: true, productContext: true },
  });
  ...
  const variants = await p.variant.findMany({
    ...
    select: { title: true, option1: true, option2: true, option3: true, position: true, featuredImageId: true },
    orderBy: { position: "asc" },
  });
  console.log(`\n${variants.length} visible variants:`);
  for (const v of variants) console.log(`  pos${v.position}  ${v.title}  | ${v.option1} | ${v.option2} | ${v.option3}  feat=${v.featuredImageId}`);

  // Existing heroes / lifestyles / closeups
  const [hero, life, closeup] = await Promise.all([
  ...
  console.log(`\nExisting: hero-flat=${hero}  lifestyle=${life}  closeup=${closeup}`);
  await p.$disconnect();
})();

/**
 * Revert variant #18 (Silver / Stainless Steel Bracelet) on cmq3nwk6g to its
 * 1688 source image so the bulk hero script picks it up as unserved, then
 * regenerates the hero using the new hero-overrides JSON (no plastic stand).
 */
...
(async () => {
  ...
  const v = await prisma.variant.findFirst({
    where: { productId: PID, position: 18 },
    select: { id: true, title: true, featuredImageId: true },
  });
  if (!v) { console.error("Variant #18 not found"); process.exit(1); }
  console.log(`Variant #18: ${v.id} "${v.title}"`);
  ...
  if (!origSource) { console.error("No 1688 source found for variant #18"); process.exit(1); }
  ...
  console.log(`\nRunning bulk hero script for ${PID} (will regen the now-unserved variant #18 with hero-override notes)...`);
  const code = await new Promise<number>((resolve) => {
    const proc = spawn(
      "npx",
      ["tsx", "scripts/_hero-image-creator.ts", PID, "--concurrency", "1"],

/**
 * One-off: generate hero images using Nano Banana Pro (Gemini 3 Pro Image)
 * via kie, with the user's Pottery-Barn-style detailed prompt. NO post-process
 * (no bg removal, no centering, no gradient bake). Uploads to Supabase as
 * imageType="hero" so they show in the review UI.
 *
 * Usage:
 *   npx dotenv -e .env.local -- npx tsx scripts/_hero-nbp-oneoff.ts <productId>
 */
...
async function callNanoBananaPro(sourceUrl: string): Promise<Buffer> {
  const createRes = await fetch(KIE_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      input: {
        prompt: HERO_PROMPT,
        image_input: [sourceUrl], // NOTE: image_input, not image_urls (NBP convention)
        aspect_ratio: "1:1",
...
async function main() {
  ...
  const settled = await Promise.allSettled(
    allGroups.map(async (group, idx) => {
      ...
      try {
        const buf = await callNanoBananaPro(group.sourceUrl);

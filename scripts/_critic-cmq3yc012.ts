/**
 * Hero critic for agent-mode product cmq3yc012 (833685568204) — watches rubric.
 * Adapted from _critic-prototype-watches.ts. Read-only.
 */
...
H) NO_STAND: absolutely no transparent/clear acrylic display stand, plastic watch holder, perspex prop, plinth, riser, or any visible support under or behind the case — watch should appear to float with only a faint contact shadow

Return:
{"A":"pass|fail","A_detail":"","B":"pass|fail","B_detail":"","C":"pass|fail","C_detail":"","D":"pass|fail","D_detail":"","E":"pass|fail","E_detail":"","F":"pass|fail","F_detail":"","G":"pass|fail","G_detail":"","H":"pass|fail","H_detail":"","overall":"clean|minor|major"}`;
...
(async () => {
  ...
  console.log("\n========== SUMMARY ==========");
  const counts: Record<string, number> = { clean: 0, minor: 0, major: 0, error: 0 };
  for (const r of results) {
    if (r.error) counts.error++;
    else if (r.rubric) counts[r.rubric.overall] = (counts[r.rubric.overall] ?? 0) + 1;
  }
  console.log(`clean=${counts.clean}  minor=${counts.minor}  major=${counts.major}  error=${counts.error}`);

  console.log("\n========== PER-RULE FAILURES ==========");
  const ruleNames: Record<string, string> = { A: "NO_TAG", B: "NO_PROP", C: "UPRIGHT", D: "LEGIBLE", E: "NO_PEOPLE", F: "NO_TEXT", G: "SCALE", H: "NO_STAND" };
  for (const key of Object.keys(ruleNames)) {
    const fails = results.filter((r) => r.rubric && (r.rubric as any)[key] === "fail");
    if (fails.length === 0) continue;
    console.log(`\n${key} (${ruleNames[key]}) — ${fails.length} fail(s):`);
    for (const f of fails) {
      const detailKey = `${key}_detail`;
      console.log(`  #${f.variant?.position ?? "?"} ${f.variant?.title ?? "—"}: ${(f.rubric as any)![detailKey]}`);
    }
  }

  const out = path.resolve(process.cwd(), ".tmp-classify", `critic-${PID}.json`);
  fs.mkdirSync(path.dirname(out), { recursive: true });
  fs.writeFileSync(out, JSON.stringify(results.map((r) => ({
    heroId: r.hero.id,
    variantPosition: r.variant?.position,
    variantTitle: r.variant?.title,
    sourceUrl: r.hero.sourceUrl,
    rubric: r.rubric,
    error: r.error,
  })), null, 2));
  console.log(`\nWrote ${out}`);

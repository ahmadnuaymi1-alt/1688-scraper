/** Set productType=jewelry-box for cmq48cyg3 (sequential single write). */
...
async function main() {
  ...
  const before = await prisma.product.findUnique({ where: { id: ID }, select: { productType: true } });
  ...
  console.log("after  productType:", JSON.stringify(after?.productType));

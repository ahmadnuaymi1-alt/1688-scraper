 ...
 * for the 9 products uploaded to Vilvida on 2026-05-31 17:01 and push them via
 * Shopify's metafieldsSet mutation.
 *
 * Pattern + style guide copied verbatim from _fill-may29-metafields.ts which
 * itself was reverse-engineered from the 5/25 wall-sconce uploads via
 * _inspect-vilvida-metafields.ts. Same model (claude-sonnet-4-6), same JSON
 * shape, same metafield types (note faq_q3 is multi_line_text_field per the
 * store's existing schema), same parallel fanout via Promise.all.
 *
 * Pre-flight: refreshes the Shopify Admin token via OAuth client_credentials
 * because shpca_* tokens expire at 24h and the prior price-rounding run was
 * the last successful auth.
 */
...
const PRODUCT_IDS = [
  "cmpspuo8e00wvw24cpnxkhj62",
  "cmpspc34s00j0w24c40x2tikh",
  "cmpspbp9700gew24c1kfrcek7",
...
async function refreshToken(shop: string, clientId: string, clientSecret: string): Promise<string> {
  ...
  if (!j.access_token) throw new Error(`refreshToken failed: ${JSON.stringify(j)}`);
...
(async () => {
  ...
  if (conn.clientId && conn.clientSecret) {
    try {
      const fresh = await refreshToken(conn.storeDomain, conn.clientId, conn.clientSecret);
      await prisma.shopifyConnection.update({ where: { id: conn.id }, data: { accessToken: fresh } });
      token = fresh;
      console.log(`Refreshed Shopify token (${token.slice(0, 12)}…)\n`);
    } catch (e) {
      console.log(`Token refresh failed, using existing token: ${e instanceof Error ? e.message : e}\n`);
    }
  }
  ...
  const generated = await Promise.all(
    PRODUCT_IDS.map(async (id) => {
      ...
      try {
        ...
        console.log(`  GEN OK  ${id}  → ${r.shopifyGid}  (${fmt(performance.now() - tStart)})`);
  ...
  const pushed = await Promise.all(
    ok.map(async (g) => {
      const tStart = performance.now();
      const r = await pushMetafields(conn.storeDomain, token, g.shopifyGid, g.metafields);
  ...
  console.log(`\n=== Done in ${fmt(performance.now() - t0)}: generated ${ok.length}/${PRODUCT_IDS.length}, pushed ${pushOk}/${ok.length} ===`);

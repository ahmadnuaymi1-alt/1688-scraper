...
- **Model:** Higgsfield's `nano_banana_2` (Nano Banana Pro), 1K resolution, 1:1 aspect ratio.
...
- **Prompt (category-aware — analyse the product first):** the effective prompt comes from `buildHeroPrompt(productId, { title, productType })` in [src/lib/hero/verify.ts](src/lib/hero/verify.ts), which **branches on the product category** in this order:
  - **Watches / wristwatches** (matched by `isWatchProduct(title + productType)`) → `HERO_PROMPT_WATCH` — a watch-specific studio prompt: slightly warmer pale-beige backdrop (#E8DFCE), soft 3/4 angle, bracelet curving down-and-back, crystal rendered glare-free so the dial / hands / markers / date window are fully legible, brushed-vs-polished metal finishes preserved, two-tone bracelets shown with both tones per-link as in the reference. The first baked-in category override under the cornucopia pattern.
  - **Lighting fixtures** (matched by `isLightingProduct(...)`) → `HERO_PROMPT` (the bulb-aware prompt: mirror the reference bulb state, mounting-surface rules flush-mount→ceiling / sconce→wall / floor-lamp→floor / table→tabletop).
  - **Everything else (general fallback)** (jewellery boxes, decor, furniture, kitchenware, …) → `HERO_PROMPT_GENERAL` — the pale-greige studio-cove look with ALL light/bulb/mount language stripped; it just renders the reference product, unchanged, on the cove.
  - The classifiers live in `src/services/lifestyle-scene-designer.service.ts`; the prompts live in [src/lib/hero/prompt.ts](src/lib/hero/prompt.ts). When the user asks for a new category-specific prompt, add a new `HERO_PROMPT_<CATEGORY>` constant + a sister `is<Category>Product()` classifier, then extend `buildHeroPrompt()` to check the new category before the existing branches.
  - **Per-product override (escape hatch, highest precedence):** `hero-overrides/<id>.json` (`heroPromptOverride` full-replace or `heroPromptExtraNotes` append) still wins over the category branch — use it to force a specific prompt or add correction notes when the Gemini check keeps flagging a product.
- **Gemini source-match check:** after each hero, `verifyAndMaybeRegenerate` asks Gemini whether the hero matches its source and auto-redoes on a CLEAR mismatch (≤2×); runs the same regardless of lighting vs general. The script collapses the prompt's newlines into spaces (cmd.exe doesn't survive literal newlines inside quoted args).
...
- [src/lib/hero/prompt.ts](src/lib/hero/prompt.ts) — the three hero prompts: `HERO_PROMPT` (lighting), `HERO_PROMPT_GENERAL` (non-lighting fallback), `HERO_PROMPT_WATCH` (wristwatches). Edit here to change what the model is asked for; add new `HERO_PROMPT_<CATEGORY>` constants here when growing the cornucopia.
- [src/lib/hero/verify.ts](src/lib/hero/verify.ts) — `buildHeroPrompt` (the watch/lighting/general branch + override precedence) and the Gemini source-match check.
- [src/services/lifestyle-scene-designer.service.ts](src/services/lifestyle-scene-designer.service.ts) — `isLightingProduct()` and `isWatchProduct()` (keyword tests shared by hero + lifestyle branches).

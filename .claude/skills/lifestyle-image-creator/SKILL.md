...
## Lighting vs. general — analyse the product FIRST

Before anything else, decide what kind of product this is (`isLightingProduct(title + productType)` — the same test the hero skill uses):

- **Lighting fixture** → use the lighting framework documented in the rest of this file (classify category → archetype menu → dazuma-tuned scenes via `lifestyle-scene-designer`).
- **NON-lighting / general product** (jewellery box, decor, furniture, kitchenware, watch, bag, …) → DO NOT use the lighting framework (it would force a "pendant" category + room-lighting rules). Instead run the **general process** below — it authors UNIQUE scene prompts the same way the lighting designer invents its own (NOT from a fixed library), just guided by a per-category **memory recipe** instead of the dazuma archetypes.

### General (non-lighting) process

1. **Functionality analysis (per-product, OBVIOUS-only, never forced).** Look at the product's OWN reference images. Does it have functional states that visually "slap you in the face" (a box that clearly opens/closes, a tray that lifts out, a thing that folds)? ONLY depict functionality the images actually show. **NEVER invent a state the product doesn't have — fabricating it misleads customers and is the #1 failure mode.** When obviously functional, vary BOTH the state (closed / partially-open / fully-open / part-removed) AND the contents/parts across the set. Simple single-state products (a watch, a tie, a mug) → functionality is a non-factor; don't force or overdo it.
2. **Category research checklist (ONLY the first time a category is seen — otherwise load the memory recipe and skip).** Research how the category is shot for e-commerce/lifestyle: common environments, camera angles, how prominent close-ups/detail shots are, any studio/creative/"fake-studio" styles worth borrowing, typical props; and the category's functional conventions. Distil into a recipe — to GUIDE unique prompts, not copy verbatim.
3. **Save to memory (cornucopia).** Write `memory/lifestyle-cat-<slug>.md` (type `reference`) + a `MEMORY.md` pointer; see `lifestyle-cat-jewellery-box.md` for the format. On repeat scrapes of a known category, load the recipe and skip step 2.
4. **Author 6–8 UNIQUE scene-overrides** in `scene-overrides/<id>.json` (format = `lifestyle-scene-designer`). Reference-image-only anchor (no embedded product description), medium-density lived-in editorial scenes, functional-state + angle/style variety guided by the recipe, no people, no alcohol. Pull realism technique from the `anti-ai-image-realism` skill. The override path generates exactly your scene count, clamped **6–8**.
5. **Generate** via `_lifestyle-image-creator.ts` (consumes the override) + the closeup step; for close-up-heavy categories run up to 2 closeups (`_hf-cli-bulk-closeups.ts --count 2`).

The mechanics below (variant rotation, Higgsfield CLI, Supabase attach, idempotency) are **identical for both branches** — only the SCENE SOURCE differs (lighting archetypes vs. hand-authored general overrides). The detailed general procedure also lives in the [general-lifestyle-process] memory.
